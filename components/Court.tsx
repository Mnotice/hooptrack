...
import type { Shot, ShotAnimation, ZoneId } from '@/lib/types';
...
interface ZoneAnim {
  ...
  missVariant: { type: string; drift?: number; reboundX?: number } | null;
}

import { useEffect, useState } from 'react'
import { ZONE_ORIGINS, getMissVariant } from '@/lib/zoneOrigins'
import '../styles/Court.css'

export const ZONES: { id: ZoneId; short: string; label: string }[] = [
...
function getZoneStats(shots: Shot[], zoneId: ZoneId) {
...
export default function Court({ selectedZone, onZoneSelect, shots = [], shotAnimation = null }: CourtProps) {
  const selected = ZONES.find((z) => z.id === selectedZone)
  const [anim, setAnim] = useState<ZoneAnim | null>(null)
  ...
  const ballStyle: CSSProperties & Record<string, string> = anim ? {
