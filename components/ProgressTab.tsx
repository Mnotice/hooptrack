...
export default function ProgressTab({ sessions }: ProgressTabProps) {
  const stats = useMemo(() => {
    const now = Date.now()
    ...
    const shootingSessions = sessions.filter((s): s is import('@/lib/types').ShootingSession => s.type === 'shooting')
    const currentWeekSessions = shootingSessions.filter(s => (now - s.startTime) < oneWeekMs)
    const lastWeekSessions = shootingSessions.filter(
    ...
    const allShots = shootingSessions.flatMap(s => s.shots)
